...
{
    CDialogEx::DoDataExchange(pDX);
    DDX_Control(pDX, IDC_MFCEDITBROWSE_OPEN, m_CMFCEditBrowseCtrl);
}
...
BEGIN_MESSAGE_MAP(CPEToolDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
    ON_EN_CHANGE(IDC_MFCEDITBROWSE_OPEN, &CPEToolDlg::OnEnChangeMfceditbrowseOpen)
END_MESSAGE_MAP()
...
void CPEToolDlg::OnEnChangeMfceditbrowseOpen()
{
    ...
    CString ss;
    m_CMFCEditBrowseCtrl.GetWindowText(ss);
    TRACE(ss);
}
